...
def list_possible_P():
	...
	for i in dependency:
		for j in datacenter_in_use:#range(datacenter_number_begin, datacenter_number_end):
			for k in datacenter_in_use:# range(datacenter_number_begin, datacenter_number_end):
				for m in datacenter_in_use:#range(datacenter_number_begin, datacenter_number_end):
...
def init():
	...
	datacenter_number = len(datacenter_in_use)#19 #TODO
	if sys.argv[2] != 'ec2':
		datacenter_number_max = 19
	else:
		datacenter_number_max = 8
	...
	fin = open('/home/csgrads/zwu005/research/metaCloud/data/information/storage_latency_matrix_percentile','r')
        lines = fin.readlines()
        fin.close()
        for line in lines:
                items = line[:-1].split(' ')
                if int(items[0]) < datacenter_number_max and int(items[1]) < datacenter_number_max and items[2] == 'GET':
                        latency[int(items[0])][int(items[1])]['GET'][items[3]] = float(items[4])
                        latency[int(items[0])][int(items[1])]['PUT'][items[3]] = float(items[4])

	fin = open('/home/csgrads/zwu005/research/metaCloud/data/information/vm_latency_matrix_percentile','r')
        lines = fin.readlines()
        fin.close()
...
def in_put_sla(i,j):  # i sends data directly to j
	...
	if latency[i][j]['PUT'][percentile] <= PUT_SLA:  #here we assume that l[i,j]+l[j,k] > l[i,k] is always true
		return True
	else:
		return False

def in_put_sla_3(i,j,k):
	if latency[i][j]['VM'][percentile] + latency[j][k]['PUT'][percentile] <= PUT_SLA:
		return True
	else:
		return False
...
def output_objective_function():
	...
	for i in dependency:
		for j in datacenter_in_use:#range(datacenter_number_begin, datacenter_number_end):
			if in_put_sla(i,j) and in_get_sla(i,j):
				if need_operation:
					fout.write(' + ')
	...
	for i in datacenter_in_use:#range(datacenter_number_begin, datacenter_number_end):
		for j  in datacenter_in_use:#range(datacenter_number_begin, datacenter_number_end):
			if i != j:
				if need_operation:
					fout.write(' + ')
...
def output_one_replica_per_data_center():
	...
	for i in dependency:
		...
		for j in datacenter_in_use:#range(datacenter_number_begin, datacenter_number_end):
			if in_get_sla(i,j) and in_put_sla(i,j):
				if need_operation:
					fout.write(' + ')
				else:
...
def output_direct_forward_to_chosen_replica():
	...
	for i in dependency:
		for k in datacenter_in_use:#range(datacenter_number_begin, datacenter_number_end):
			if in_put_sla(i,k) and in_get_sla(i,k):
				need_operation = False
				for j in datacenter_in_use:#range(datacenter_number_begin, datacenter_number_end):
